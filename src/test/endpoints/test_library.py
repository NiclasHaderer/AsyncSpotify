...
class TestLibrary:
    ...
    async def test_contains(self, prepared_api: SpotifyApiClient):
        ...
        assert not album
        c_album = await prepared_api.library.contains_album(['07bYtmE3bPsLB6ZbmmFi8d', '48JYNjh7GMie6NjqYHMmtT'])
        assert c_album[0]
        album = await prepared_api.library.remove_album(['07bYtmE3bPsLB6ZbmmFi8d', '48JYNjh7GMie6NjqYHMmtT'])
        assert not album
        c_album = await prepared_api.library.contains_album(['07bYtmE3bPsLB6ZbmmFi8d', '48JYNjh7GMie6NjqYHMmtT'])
        ...
        assert not shows
        c_shows = await prepared_api.library.contains_show(['5AvwZVawapvyhJUIx71pdJ', '6ups0LMt1G8n81XLlkbsPo'])
        assert c_shows[0]
        ...
        assert not tracks
        c_tracks = await prepared_api.library.contains_track(['7ouMYWpwJ422jRcDASZB7P', '4VqPOruhp5EdPBeR92t6lQ'])
        assert c_tracks[0]
