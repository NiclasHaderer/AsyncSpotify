...
class TestArtist:
    ...
    async def test_top_tracks(self, prepared_api: SpotifyApiClient):
        artist_top = await prepared_api.artist.get_top_tracks('1YEGETLT2p8k97LIo3deHL', 'DE')
        assert isinstance(artist_top, dict) and artist_top

    @pytest.mark.asyncio
    async def test_similar(self, prepared_api: SpotifyApiClient):
        artist_similar = await prepared_api.artist.get_similar('1YEGETLT2p8k97LIo3deHL')
